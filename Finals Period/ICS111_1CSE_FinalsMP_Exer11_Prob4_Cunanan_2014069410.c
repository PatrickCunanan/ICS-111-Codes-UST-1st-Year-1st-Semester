...
#define menuItems1 8
	...
	int main()
	{
	    ...
	    while(ordering)
	    {
	        printf("Please enter the number that assign on each item that you would like to order\nand if you want another order for a particular item just enter the number again.Enter [0] if the order is complete.\n");
	        scanf("%d", &orderChoice);
	...
	void printCheck(struct menuItemType menuList[], int menuOrder[], int menuItems)
	{
	    ...
	    {
			if(menuOrder[i] > 0) {
				printf("%d %s    $%.2lf\n", menuOrder[i], menuList[i].menuItem, (menuList[i].menuPrice * menuOrder[i]));
